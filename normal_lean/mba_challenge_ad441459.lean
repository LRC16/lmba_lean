...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $-11\cdot \lnot (x\land \lnot y)+4\cdot \lnot (x\oplus y)+4\cdot (x\oplus y)+8\cdot \lnot (x\lor y)+14\cdot \lnot (x\lor \lnot y)+3\cdot (x\land \lnot y)+13\cdot (x\land y)$, $1\cdot \lnot (x\land y)+6\cdot (x\lor y)$
-/ 
theorem mba_challenge_ad441459 (x y : BitVec 32) :  -11#32 * ~~~(x &&& ~~~y) + 4#32 * ~~~(x ^^^ y) + 4#32 * (x ^^^ y) + 8#32 * ~~~(x ||| y) + 14#32 * ~~~(x ||| ~~~y) + 3#32 * (x &&& ~~~y) + 13#32 * (x &&& y) = 1#32 * ~~~(x &&& y) + 6#32 * (x ||| y) := by
  sorry
