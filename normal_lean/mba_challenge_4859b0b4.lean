...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $11\cdot \lnot (x\land \lnot y)+1\cdot (x\lor y)-1\cdot (x\lor \lnot y)-10\cdot \lnot (x\lor y)-11\cdot (x\land y)$, $12\cdot \lnot (x\lor \lnot y)$
-/ 
theorem mba_challenge_4859b0b4 (x y : BitVec 32) :  11#32 * ~~~(x &&& ~~~y) + 1#32 * (x ||| y) - 1#32 * (x ||| ~~~y) - 10#32 * ~~~(x ||| y) - 11#32 * (x &&& y) = 12#32 * ~~~(x ||| ~~~y) := by
  sorry
