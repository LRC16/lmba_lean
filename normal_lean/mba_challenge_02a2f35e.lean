...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $2\cdot \lnot (x\land \lnot y)-1\cdot (x\lor \lnot y)-7\cdot \lnot (x\oplus y)+11\cdot \lnot (x\land y)-5\cdot \lnot (x\lor y)-13\cdot \lnot (x\lor \lnot y)+6\cdot (x\land y)$, $10\cdot (x\land \lnot y)$
-/ 
theorem mba_challenge_02a2f35e (x y : BitVec 32) :  2#32 * ~~~(x &&& ~~~y) - 1#32 * (x ||| ~~~y) - 7#32 * ~~~(x ^^^ y) + 11#32 * ~~~(x &&& y) - 5#32 * ~~~(x ||| y) - 13#32 * ~~~(x ||| ~~~y) + 6#32 * (x &&& y) = 10#32 * (x &&& ~~~y) := by
  sorry
