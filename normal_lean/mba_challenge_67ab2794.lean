...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $-3\cdot (x\lor y)-1\cdot (x\lor \lnot y)+1\cdot \lnot (x\lor y)+4\cdot (x\land \lnot y)$, $-4\cdot (x\land y)-3\cdot \lnot (x\lor \lnot y)$
-/ 
theorem mba_challenge_67ab2794 (x y : BitVec 32) :  -3#32 * (x ||| y) - 1#32 * (x ||| ~~~y) + 1#32 * ~~~(x ||| y) + 4#32 * (x &&& ~~~y) = -4#32 * (x &&& y) - 3#32 * ~~~(x ||| ~~~y) := by
  sorry
