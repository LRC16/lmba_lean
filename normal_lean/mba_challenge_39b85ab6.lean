...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $7\cdot \lnot (x\oplus y)+1\cdot (x\lor y)+1\cdot \lnot (x\land y)+1\cdot (x\oplus y)-7\cdot (x\lor \lnot y)-1\cdot \lnot (x\lor y)-3\cdot \lnot (x\lor \lnot y)-1\cdot (x\land y)$, $-4\cdot (x\land \lnot y)$
-/ 
theorem mba_challenge_39b85ab6 (x y : BitVec 32) :  7#32 * ~~~(x ^^^ y) + 1#32 * (x ||| y) + 1#32 * ~~~(x &&& y) + 1#32 * (x ^^^ y) - 7#32 * (x ||| ~~~y) - 1#32 * ~~~(x ||| y) - 3#32 * ~~~(x ||| ~~~y) - 1#32 * (x &&& y) = -4#32 * (x &&& ~~~y) := by
  sorry
