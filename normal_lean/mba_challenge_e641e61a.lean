...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $2\cdot (x\lor \lnot y)+1\cdot \lnot (x\oplus y)+1\cdot \lnot (x\land \lnot y)-4\cdot \lnot (x\lor y)-1\cdot \lnot (x\lor \lnot y)-2\cdot (x\land \lnot y)$, $4\cdot (x\land y)$
-/ 
theorem mba_challenge_e641e61a (x y : BitVec 32) :  2#32 * (x ||| ~~~y) + 1#32 * ~~~(x ^^^ y) + 1#32 * ~~~(x &&& ~~~y) - 4#32 * ~~~(x ||| y) - 1#32 * ~~~(x ||| ~~~y) - 2#32 * (x &&& ~~~y) = 4#32 * (x &&& y) := by
  sorry
