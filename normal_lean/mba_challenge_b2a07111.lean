...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $1\cdot (x\lor \lnot y)+7\cdot (x\lor y)+11\cdot \lnot (x\land y)-6\cdot \lnot (x\land \lnot y)-6\cdot \lnot (x\lor y)-18\cdot (x\land \lnot y)-2\cdot (x\land y)$, $11\cdot \lnot (x\lor \lnot y)+1\cdot (x\oplus y)$
-/ 
theorem mba_challenge_b2a07111 (x y : BitVec 32) :  1#32 * (x ||| ~~~y) + 7#32 * (x ||| y) + 11#32 * ~~~(x &&& y) - 6#32 * ~~~(x &&& ~~~y) - 6#32 * ~~~(x ||| y) - 18#32 * (x &&& ~~~y) - 2#32 * (x &&& y) = 11#32 * ~~~(x ||| ~~~y) + 1#32 * (x ^^^ y) := by
  sorry
