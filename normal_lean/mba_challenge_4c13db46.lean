...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $3\cdot (x\oplus y)-2\cdot (x\lor \lnot y)+2\cdot \lnot (x\lor y)-1\cdot (x\land \lnot y)$, $3\cdot \lnot (x\lor \lnot y)-2\cdot (x\land y)$
-/ 
theorem mba_challenge_4c13db46 (x y : BitVec 32) :  3#32 * (x ^^^ y) - 2#32 * (x ||| ~~~y) + 2#32 * ~~~(x ||| y) - 1#32 * (x &&& ~~~y) = 3#32 * ~~~(x ||| ~~~y) - 2#32 * (x &&& y) := by
  sorry
