...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $-1\cdot (x\lor \lnot y)-2\cdot \lnot (x\land \lnot y)+4\cdot \lnot (x\lor \lnot y)+3\cdot (x\land \lnot y)+3\cdot (x\land y)$, $-3\cdot \lnot (x\lor y)+2\cdot (x\oplus y)$
-/ 
theorem mba_challenge_b4a1e5f0 (x y : BitVec 32) :  -1#32 * (x ||| ~~~y) - 2#32 * ~~~(x &&& ~~~y) + 4#32 * ~~~(x ||| ~~~y) + 3#32 * (x &&& ~~~y) + 3#32 * (x &&& y) = -3#32 * ~~~(x ||| y) + 2#32 * (x ^^^ y) := by
  sorry
