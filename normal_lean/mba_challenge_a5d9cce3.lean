...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $-6\cdot \lnot (x\land y)-1\cdot \lnot (x\land \lnot y)-5\cdot (x\lor \lnot y)+1\cdot \lnot (x\oplus y)+11\cdot \lnot (x\lor y)+9\cdot (x\land \lnot y)+3\cdot (x\land y)$, $-5\cdot \lnot (x\lor \lnot y)-2\cdot (x\lor y)$
-/ 
theorem mba_challenge_a5d9cce3 (x y : BitVec 32) :  -6#32 * ~~~(x &&& y) - 1#32 * ~~~(x &&& ~~~y) - 5#32 * (x ||| ~~~y) + 1#32 * ~~~(x ^^^ y) + 11#32 * ~~~(x ||| y) + 9#32 * (x &&& ~~~y) + 3#32 * (x &&& y) = -5#32 * ~~~(x ||| ~~~y) - 2#32 * (x ||| y) := by
  sorry
