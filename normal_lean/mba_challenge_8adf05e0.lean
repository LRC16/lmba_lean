...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $-1\cdot \lnot (x\land y)-1\cdot \lnot (x\oplus y)-1\cdot (x\oplus y)-7\cdot \lnot (x\land \lnot y)+2\cdot \lnot (x\lor y)+11\cdot \lnot (x\lor \lnot y)-3\cdot (x\land \lnot y)+3\cdot (x\land y)$, $-7\cdot (x\lor \lnot y)+2\cdot (x\lor y)$
-/ 
theorem mba_challenge_8adf05e0 (x y : BitVec 32) :  -1#32 * ~~~(x &&& y) - 1#32 * ~~~(x ^^^ y) - 1#32 * (x ^^^ y) - 7#32 * ~~~(x &&& ~~~y) + 2#32 * ~~~(x ||| y) + 11#32 * ~~~(x ||| ~~~y) - 3#32 * (x &&& ~~~y) + 3#32 * (x &&& y) = -7#32 * (x ||| ~~~y) + 2#32 * (x ||| y) := by
  sorry
