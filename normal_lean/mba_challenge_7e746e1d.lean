...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $-1\cdot \lnot (x\oplus y)+4\cdot (x\lor \lnot y)+11\cdot \lnot (x\land \lnot y)+11\cdot (x\oplus y)+3\cdot \lnot (x\land y)-17\cdot \lnot (x\lor y)-30\cdot \lnot (x\lor \lnot y)-19\cdot (x\land y)$, $-5\cdot (x\lor y)+23\cdot (x\land \lnot y)$
-/ 
theorem mba_challenge_7e746e1d (x y : BitVec 32) :  -1#32 * ~~~(x ^^^ y) + 4#32 * (x ||| ~~~y) + 11#32 * ~~~(x &&& ~~~y) + 11#32 * (x ^^^ y) + 3#32 * ~~~(x &&& y) - 17#32 * ~~~(x ||| y) - 30#32 * ~~~(x ||| ~~~y) - 19#32 * (x &&& y) = -5#32 * (x ||| y) + 23#32 * (x &&& ~~~y) := by
  sorry
