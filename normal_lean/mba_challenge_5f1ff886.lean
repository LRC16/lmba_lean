...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $3\cdot \lnot (x\land \lnot y)+2\cdot \lnot (x\land y)-5\cdot \lnot (x\lor \lnot y)-6\cdot (x\land \lnot y)-7\cdot (x\land y)$, $9\cdot \lnot (x\lor y)-4\cdot (x\lor \lnot y)$
-/ 
theorem mba_challenge_5f1ff886 (x y : BitVec 32) :  3#32 * ~~~(x &&& ~~~y) + 2#32 * ~~~(x &&& y) - 5#32 * ~~~(x ||| ~~~y) - 6#32 * (x &&& ~~~y) - 7#32 * (x &&& y) = 9#32 * ~~~(x ||| y) - 4#32 * (x ||| ~~~y) := by
  sorry
