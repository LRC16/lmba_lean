...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $7\cdot \lnot (x\land \lnot y)+2\cdot (x\lor y)+11\cdot (x\oplus y)+4\cdot \lnot (x\oplus y)-1\cdot \lnot (x\land y)-9\cdot \lnot (x\lor y)-11\cdot (x\land \lnot y)-12\cdot (x\land y)$, $19\cdot \lnot (x\lor \lnot y)+1\cdot (x\lor \lnot y)$
-/ 
theorem mba_challenge_4a72dcdb (x y : BitVec 32) :  7#32 * ~~~(x &&& ~~~y) + 2#32 * (x ||| y) + 11#32 * (x ^^^ y) + 4#32 * ~~~(x ^^^ y) - 1#32 * ~~~(x &&& y) - 9#32 * ~~~(x ||| y) - 11#32 * (x &&& ~~~y) - 12#32 * (x &&& y) = 19#32 * ~~~(x ||| ~~~y) + 1#32 * (x ||| ~~~y) := by
  sorry
