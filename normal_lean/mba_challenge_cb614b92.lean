...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $4\cdot \lnot (x\land \lnot y)+3\cdot (x\lor y)-2\cdot (x\lor \lnot y)-7\cdot \lnot (x\oplus y)+5\cdot \lnot (x\lor y)-1\cdot (x\land \lnot y)+2\cdot (x\land y)$, $7\cdot \lnot (x\lor \lnot y)$
-/ 
theorem mba_challenge_cb614b92 (x y : BitVec 32) :  4#32 * ~~~(x &&& ~~~y) + 3#32 * (x ||| y) - 2#32 * (x ||| ~~~y) - 7#32 * ~~~(x ^^^ y) + 5#32 * ~~~(x ||| y) - 1#32 * (x &&& ~~~y) + 2#32 * (x &&& y) = 7#32 * ~~~(x ||| ~~~y) := by
  sorry
