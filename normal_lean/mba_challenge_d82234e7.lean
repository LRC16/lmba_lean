...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $2\cdot (x\lor y)-7\cdot \lnot (x\land \lnot y)-5\cdot \lnot (x\land y)+1\cdot (x\lor \lnot y)+4\cdot (x\oplus y)-7\cdot \lnot (x\oplus y)+18\cdot \lnot (x\lor y)+11\cdot (x\land y)$, $2\cdot (x\land \lnot y)-6\cdot \lnot (x\lor \lnot y)$
-/ 
theorem mba_challenge_d82234e7 (x y : BitVec 32) :  2#32 * (x ||| y) - 7#32 * ~~~(x &&& ~~~y) - 5#32 * ~~~(x &&& y) + 1#32 * (x ||| ~~~y) + 4#32 * (x ^^^ y) - 7#32 * ~~~(x ^^^ y) + 18#32 * ~~~(x ||| y) + 11#32 * (x &&& y) = 2#32 * (x &&& ~~~y) - 6#32 * ~~~(x ||| ~~~y) := by
  sorry
