...
theorem bv32_sum_all (x y : BitVec 32) :
  ...
  bv_decide
/-- 
Let x,y be 32-bit bit-vectors. Prove the equivalence of the following two expressions: $-7\cdot \lnot (x\oplus y)-2\cdot (x\lor \lnot y)-6\cdot (x\oplus y)+1\cdot \lnot (x\land \lnot y)+8\cdot \lnot (x\lor y)+8\cdot (x\land y)$, $-5\cdot \lnot (x\lor \lnot y)-8\cdot (x\land \lnot y)$
-/ 
theorem mba_challenge_20e8b21d (x y : BitVec 32) :  -7#32 * ~~~(x ^^^ y) - 2#32 * (x ||| ~~~y) - 6#32 * (x ^^^ y) + 1#32 * ~~~(x &&& ~~~y) + 8#32 * ~~~(x ||| y) + 8#32 * (x &&& y) = -5#32 * ~~~(x ||| ~~~y) - 8#32 * (x &&& ~~~y) := by
  sorry
